...
const SYSTEM_ASSETS = {
    // 1. PERFORMANCE OPTIMIZED CSS (Zero Layout Shift + Category Grid)
...
/* Featured Image - 1200x628 Aspect Ratio */
...
.share-btn{display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:4px;color:#fff;font-weight:600;font-size:0.9rem;transition:opacity 0.2s}
...
@media screen and (max-width:768px){
    .nav-links{position:fixed;top:0;right:0;width:100%;height:100vh;background:rgba(18,18,18,0.98);backdrop-filter:blur(10px);display:flex;flex-direction:column;align-items:center;justify-content:center;transform:translateX(100%);transition:transform 0.4s cubic-bezier(0.4, 0, 0.2, 1);z-index:2000;will-change:transform;gap: 2rem;}
...
const state = { token: null, owner: null, repo: null, currentType: 'post', currentSlug: null, currentSha: null, settings: {}, settingsSha: null, contentIndex: [], indexSha: null };
...
async function initApp() {
    ...
    switchPanel('dashboard'); showLoader(false);
}
...
document.getElementById('save-btn').addEventListener('click', async () => {
    ...
    // Featured Img (1200x628 container)
    ...
    // WRITE-TIME SEO CALCULATION (Zero Layout Shift)
    ...
    // BREADCRUMB LOGIC (Structure Toggle)
    let crumbHtml = '';
    if(isPost) {
...
// GENERATE CATEGORY PAGES (Physical Files, Professional Grid)
async function generateCategoryPages(s, headerLinks, footerLinks, globalSocials) {
    ...
    for (const cat of categories) {
        ...
        if(catPosts.length === 0) gridHtml = '<p style="text-align:center;padding:50px;">No posts found.</p>';
            ...
            catPosts.forEach(p => {
                const img = await getFeaturedImageFor(p.slug); // Helper needed or assume from metadata? 
                // Optimization: We don't store image in index to save size. We'll use a placeholder or check HTML.
                // For speed, let's skip fetching every post HTML. We'll add 'image' to index in future update.
                // For now, use site favicon or generic placeholder if not in index. 
...
async function generateSitemaps(s) {
    ...
    // 2. News Sitemap (Last 48h + News Category)
    if(s.newsCategory) {
...
async function loadSidebarMedia(){
    ...
    try {
        ...
        // SORT DESCENDING (Name contains timestamp)
...
// Missing Init Helpers
function githubReq(endpoint, method = 'GET', body = null) { const url = `https://api.github.com/repos/${state.owner}/${state.repo}/${endpoint}${method === 'GET' ? '?t='+Date.now() : ''}`; const opts = { method, headers: { Authorization: `Bearer ${state.token}`, Accept: 'application/vnd.github.v3+json', 'Content-Type': 'application/json' } }; if (body) opts.body = JSON.stringify(body); return fetch(url, opts).then(r=>{if(!r.ok && method==='GET' && r.status===404)return null; if(!r.ok) throw new Error('API Error'); return r;}); }
